...
struct PointerState {
    std::uint8_t bits = kStateUnknown;

    bool empty() const { return bits == kStateUnknown; }
    bool hasNull() const { return (bits & kStateNull) != 0; }
    bool hasRaw() const { return (bits & kStateRaw) != 0; }
    bool hasManagedObject() const { return (bits & kStateManagedObject) != 0; }
    bool hasManagedArray() const { return (bits & kStateManagedArray) != 0; }
...
class ManagedStateAnnotator {
public:
    explicit ManagedStateAnnotator(llvm::Module &module) : module_(module) {
...
private:
    ...
    PointerState getCallResultState(const llvm::Function &callee) {
        ...
        return functionStates_[&callee].returnState;
    }

    PointerState getValueState(llvm::Value &value) const {
    ...
    llvm::MDNode *createStateNode(PointerState state) const {
        ...
        return llvm::MDNode::get(
            context, llvm::MDString::get(context, renderState(state)));
    }
...
llvm::Error annotateManagedState(llvm::Module &module) {
    ManagedStateAnnotator annotator(module);
